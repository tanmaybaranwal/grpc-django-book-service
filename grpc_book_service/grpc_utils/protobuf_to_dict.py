...
def protobuf_to_dict(pb, type_callable_map=TYPE_CALLABLE_MAP,
    ...
    for field, value in pb.ListFields():
        ...
        if field.type == FieldDescriptor.TYPE_ENUM and \
                field.enum_type.name == "Bool":
            if value == 1:
                value = True
            elif value == 2:
                value = False
        ...
        if field.type == FieldDescriptor.CPPTYPE_STRING and value == "Nil":
            # Custom handles for String fields receiving 'Nil' value, which is
            # being mimicked by us to handle default
            # values for string in proto buf, i.e., empty string ("")
            value = ""
        elif field.type == FieldDescriptor.CPPTYPE_STRING and value == "None":
            # Custom handles for String fields receiving 'None' value, which is
            # being mimicked by us to handle Null
            # values for string
            value = None
...
def dict_to_protobuf(pb_klass_or_instance, values,
                     type_callable_map=REVERSE_TYPE_CALLABLE_MAP, strict=True,
                     ignore_none=False):
    """Populates a protobuf model from a dictionary.

    :param pb_klass_or_instance: a protobuf message class, or an protobuf instance
    :type pb_klass_or_instance: a type or instance of a subclass of
            google.protobuf.message.Message
    :param values: a dictionary of values. Repeated and nested values are
       fully supported.
    ...
    :param ignore_none: should ignore null field or not
    """
...
def _dict_to_protobuf(pb, value, type_callable_map, strict, ignore_none):
    ...
    for field, input_value, pb_value in fields:
            ...
            if field.message_type and field.message_type.has_options and field.message_type.GetOptions().map_entry:
                ...
                for key, value in input_value.items():
                    if value_field.cpp_type == FieldDescriptor.CPPTYPE_MESSAGE:
                        _dict_to_protobuf(getattr(pb, field.name)[key], value,
                                          type_callable_map, strict,
                                          ignore_none)
                    else:
        ...
        if field.type == FieldDescriptor.TYPE_ENUM and type(
                input_value) == bool:
...
def validate_dict_for_required_pb_fields(pb, dic):
    ...
    for field, field_name, field_options in get_field_names_and_options(pb):
        if not field_options.get('is_optional',
                                 False) and field_name not in dic:
